...
async function init() {
  ...
  if (!api_key) {
    terminal.red(
      'Error: OpenAI API key not found. Please set OPENAI_API_KEY in your environment variables or in the config.json file.\n'
    )
  ...
  while (true) {
    ...
    for await (const chunk of response.stream) {
      ...
      if (last_height < terminal.height) {
        ...
        terminal.eraseDisplayBelow()
        terminal(
          rendered_response_so_far
            .split('\n')
            .slice(-(terminal.height - 2))
            .join('\n')
        )
      }
    }
