...
export interface GatherContext {
  updateStatus: (status: string, options?: { level?: 'success' | 'info' | 'alert' }) => void
...
export async function gatherInputs(inputs: string[], cwd = process.cwd()): Promise<GatherResult> {
  ...
  for (const i in inputs) {
    ...
    function updateStatus(status: string, { level = 'info' }: Parameters<GatherContext['updateStatus']>[1] = {}) {
      ...
      statusScreenBuffer.put({ ...putOptions, attr: { color } }, `\`${input}\` - ${status}`) // Update status
