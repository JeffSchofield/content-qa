...
async function processVideoContent(page: Page, { updateStatus }: GatherContext) {
  ...
  try {
    ...
    await createYTDLProcess([videoUrl, ...commonArgs, '--write-sub'])
    ...
    try {
      updateStatus('Regular subtitles not available. Checking for auto-generated subtitles...', { level: 'info' })

      // If regular subtitles fail, try auto-generated subtitles
      await createYTDLProcess([videoUrl, ...commonArgs, '--write-auto-sub'])
...
async function extractArticleContent(page: Page, { updateStatus }: GatherContext) {
  updateStatus('Extracting article content...')

  const content = await page.evaluate(() => {
    const article = document.querySelector<HTMLElement>('article, .post-content, .entry-content, main')
